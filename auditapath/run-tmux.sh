...
tmux send-keys -t $SESSION:0.0 'cd blockchain && npx hardhat node' C-m
sleep 5
...
tmux send-keys -t $SESSION:0.1 'cd blockchain && npx hardhat ignition deploy ignition/modules/PoT.js --network localhost' C-m
sleep 5
